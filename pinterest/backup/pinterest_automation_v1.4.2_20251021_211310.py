#!/usr/bin/env python3
"""
Pinterest Downloader Automation Script v1.4.2
...
# Raw destination used by gallery-dl (script will organize files from here)
RAW_DIR = DOWNLOAD_BASE / "ptdl_raw"

# gallery-dl config file to embed (stored in BASE_DIR)
CONFIG_FILE = BASE_DIR / "config.json"

# Embedded gallery-dl config (base-directory will follow RAW_DIR)
EMBEDDED_CONFIG = {
    "extractor": {
        "pinterest": {
            "username": "",
    ...
    "output": {
        # Gallery-dl will deposit downloads here; script will move & rename into PHOTOS_DIR / VIDEOS_DIR
...
def shorten_path(path: Path) -> str:
    """Shorten path by replacing base with ~~"""
...
def extract_version_from_filename(filename: str) -> tuple:
    ...
    try:
        if 'pinterest_automation_v' in filename:
            version_part = filename.split('pinterest_automation_v', 1)[1].replace('.py', '')
            # keep only numeric parts
            parts = [p for p in version_part.split('.') if p.isdigit()]
...
def backup_all_scripts():
    ...
    if latest_script:
        ...
        print(f"{BOLD} Latest Version: {RED}None{RESET}")

    backup_success = []
    backup_failed = []

    # Backup all scripts with timestamps
    print(f"\n{BOLD} Backing Up Scripts...{RESET}")
    ...
    print(f"\n{GRAY} Cleaning Up Old Versions...{RESET}")
    removed_count = 0
    for script_file in script_files:
...
def ensure_gallery_dl_config():
    ...
    try:
        if not CONFIG_FILE.exists():
            # write the embedded JSON config
            with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
                json.dump(EMBEDDED_CONFIG, f, indent=2, ensure_ascii=False)
            print(f" Created gallery-dl config: {shorten_path(CONFIG_FILE)}")
        else:
            # don't overwrite existing config; just inform
            print(f"  gallery-dl config already exists: {shorten_path(CONFIG_FILE)}")
        return True
    except Exception as e:
        print(f" Failed to create gallery-dl config: {e}")
        return False
...
def check_installation():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(" gallery-dl is installed but not working properly")
            return False
    except (subprocess.TimeoutExpired, FileNotFoundError):
        print(" gallery-dl not found. Please install with: pip install --upgrade gallery-dl")
        return False
    ...
    for file in existing_files:
        try:
            ...
            if stem.startswith(username + "_"):
                ...
                # if the num part has other characters, strip to digits prefix
                digits = ''
                for ch in num_part:
                    if ch.isdigit():
                        digits += ch
...
def run_command(command: List[str], timeout: int = 300) -> Dict[str, Any]:
    """Run an external command with proper error handling and capture"""
    try:
    ...
    if COOKIES_FILE.exists():
        try:
            ...
            if cookies_data and len(cookies_data) > 0:
                ...
                print("  Cookies file exists but appears empty")
                return False
        except json.JSONDecodeError:
            print("  Cookies file exists but is not valid JSON")
            return False
    else:
...
def download_content(url: str, username: str, use_cookies: bool = False) -> Dict[str, Any]:
    ...
    # Check for shortened URLs and warn
    if 'pin.it' in url:
        print(f"  Warning: Shortened 'pin.it' URLs may not always resolve; prefer full pinterest.com URLs")
    
    # Create a temporary download directory for this URL (inside RAW_DIR)
    timestamp_suffix = int(time.time())
    temp_dir = RAW_DIR / f"temp_{timestamp_suffix}"
    ...
    command = ["gallery-dl", "--config", str(CONFIG_FILE)]
    ...
    # Destination and URL
    command.extend(["--destination", str(temp_dir), url])
    ...
    if result['success']:
        ...
        try:
            # remove any leftover files/folders in temp_dir if empty
            if temp_dir.exists():
                shutil.rmtree(temp_dir)
            # If RAW_DIR empty, leave it (gallery-dl base dir)
        ...
        print(f" Download failed: {error_msg.strip()}")
...
def save_detailed_report(download_results: List, duration: float):
    ...
    if any(not r['success'] for r in download_results):
        ...
        with open(failed_file, 'w', encoding='utf-8') as f:
            f.write('\n'.join(failed_urls))
        print(f" Failed URLs saved to: {shorten_path(failed_file)}")
...
def display_banner():
    ...
    print(f"{RED}" + "="*50 + "" + RESET)
    print(f" Base Directory: {shorten_path(BASE_DIR)}")
    print(f" Backup Directory: {shorten_path(BACKUP_DIR)}")
    print(f" Photos: {PHOTOS_DIR}")
    print(f" Videos: {VIDEOS_DIR}")
    print(f" Cookies: {shorten_path(COOKIES_FILE)}")
    print(f"{RED}" + "="*50 + "" + RESET)
...
def main():
    ...
    parser.add_argument('--list-scripts', action='store_true',
                       help='List all script versions in base and backup directories')
    parser.add_argument('--backup-scripts', action='store_true',
                       help='Backup all scripts to backup directory and remove old versions')
    ...
    # Display banner for main functionality
    display_banner()
    
    # Ensure directories exist
    ensure_directories()

    # Ensure gallery-dl config is present (embedded)
    ...
    print(f"\n  About to download {len(urls)} URLs.")
    if use_cookies:
        print(" Using cookies for authentication")
    else:
        print("  Using public access (no cookies)")
