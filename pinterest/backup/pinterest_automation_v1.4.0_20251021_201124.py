...
def run_pinterest_dl_command(command: List[str], timeout: int = 300) -> Dict[str, Any]:
    ...
    try:
        ...
        # DEBUG: Print the actual output from pinterest-dl
        print(f"📄 pinterest-dl stdout: {result.stdout}")
        if result.stderr:
            print(f"⚠️  pinterest-dl stderr: {result.stderr}")
...
def download_content(url: str, username: str, use_cookies: bool = False) -> Dict[str, Any]:
    ...
    if 'pin.it' in url:
        print(f"⚠️  Warning: Shortened 'pin.it' URLs may not work with pinterest-dl")
        print(f"💡 Tip: Use the full Pinterest URL instead")
