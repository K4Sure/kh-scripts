#!/usr/bin/env python3
"""
Pinterest Downloader Automation Script v1.4.0
...
def list_script_versions():
    ...
    print(f"\n{RED}🚀 Launching Pinterest Downloader v{__version__}{RESET}")
    
    print(f"\n{RED}📚 Pinterest Downloader Script Versions{RESET}")
    print("=" * 60)
    
    # List 1: Backed up scripts in backup directory
    print(f"\n{BOLD}💾 Backed Up Scripts:{RESET}")
    print(f"{NEON_YELLOW}📍 Path: {shorten_path(BACKUP_DIR)}{RESET}")
    ...
    print(f"{NEON_YELLOW}📍 Path: {shorten_path(BASE_DIR)}{RESET}")
...
def backup_all_scripts():
    ...
    print(f"\n{RED}💾 Script Backup Manager{RESET}")
    print("=" * 50)
    
    # Ensure backup directory exists
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    print(f"{NEON_YELLOW}📍 Backup Path: {shorten_path(BACKUP_DIR)}{RESET}")
    ...
    print(f"🔍 Found {len(script_files)} script(s)")
    print(f"{RED}⭐ Latest version: {latest_script.name}{RESET}")
    ...
    for script_file in script_files:
        ...
        try:
            ...
            print(f"   ✅ {script_file.name} → {backup_filename}")
    ...
    # Summary
    print(f"\n📊 Backup Summary:")
    print(f"   ✅ Successfully backed up: {len(backup_success)} script(s)")
    print(f"   ❌ Failed to backup: {len(backup_failed)} script(s)")
    print(f"   🗑️  Removed old versions: {removed_count} script(s)")
    print(f"   {RED}⭐ Kept latest: {latest_script.name}{RESET}")
