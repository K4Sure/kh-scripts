...
# -*- coding: utf-8 -*-
"""
Pinterest Downloader Automation Script v1.4.8
Gallery-dl backend, embedded config, full backup manager, ANSI colors and emojis preserved.
Added multi-instance downloader functionality.
Place this file at:
 /data/data/com.termux/files/home/kh-scripts/pinterest/pinterest_automation_v1.4.8.py
"""
...
# Multi-instance downloader directories
MULTI_DL_LOGS = DOWNLOAD_BASE / "ptdl multi-logs"
...
def run_multi_command(folder_name: str, image_url: str, log_file: str, line_number: int, total_lines: int, use_cookies: bool = False):
    ...
    # FIX: Use the list directly without shell=True
...
def run_multi_instance_downloader(csv_file: str, log_file: str, threads: int, use_cookies: bool = False):
    """Run multi-instance gallery-dl downloader from CSV file"""
    print()
    print("=" * 60)
    ...
    MULTI_DL_LOGS.mkdir(parents=True, exist_ok=True)
    
    full_log_path = MULTI_DL_LOGS / log_file
    ...
    with open(csv_file, mode='r', newline='', encoding='utf-8') as file:
        ...
        print(f"   {BOLD}CSV File:{RESET} {NEON_YELLOW}{csv_file}{RESET}")
        print(f"   {BOLD}Log File:{RESET} {NEON_YELLOW}{full_log_path}{RESET}")
        print(f"   {BOLD}Threads:{RESET} {NEON_YELLOW}{threads}{RESET}")
    ...
    print(f"{BOLD}📝 Error log saved to:{RESET} {NEON_YELLOW}{full_log_path}{RESET}")

def interactive_multi_downloader(use_cookies: bool = False):
    """Interactive multi-instance downloader setup"""
    print()
    print("=" * 60)
    print(f"{BOLD}📥 Multi-Instance Downloader Setup{RESET}")
    print("=" * 60)
    print()
    
    # Get CSV file path
    csv_file = input(f"{BOLD}📋 Enter CSV file path (with 'title' and 'url' columns):{RESET} ").strip().replace('"', '')
    if not os.path.isfile(csv_file):
        print(f"{RED}❌ CSV file not found: {csv_file}{RESET}")
        return
    
    # Get log file name
    log_file = input(f"{BOLD}📝 Enter log file name (e.g., errors.csv):{RESET} ").strip() or "multi_dl_errors.csv"
    if not log_file.endswith('.csv'):
        log_file += '.csv'
    
    # Get number of threads
    try:
        threads = int(input(f"{BOLD}🔢 Number of concurrent instances (1-128):{RESET} ").strip() or "16")
        threads = max(1, min(128, threads))  # Limit between 1-128
    except ValueError:
        threads = 16
        print(f"{NEON_YELLOW}⚠️  Using default: 16 threads{RESET}")

    print()
    print(f"{BOLD}🎯 Starting Multi-Instance Downloader:{RESET}")
    print(f"   {BOLD}CSV:{RESET} {NEON_YELLOW}{csv_file}{RESET}")
    print(f"   {BOLD}Log:{RESET} {NEON_YELLOW}{MULTI_DL_LOGS / log_file}{RESET}")
    print(f"   {BOLD}Threads:{RESET} {NEON_YELLOW}{threads}{RESET}")
    print(f"   {BOLD}Cookies:{RESET} {NEON_YELLOW}{'Enabled' if use_cookies and COOKIES_FILE.exists() else 'Disabled'}{RESET}")
    print()

    run_multi_instance_downloader(csv_file, log_file, threads, use_cookies)

# ------------------------------------------------------------------
# [REST OF YOUR EXISTING FUNCTIONS REMAIN THE SAME]
# Archive Management, URL Expansion, Utilities, Backup, Config, etc.
...
# ---------------------------------------
# Running gallery-dl with archive support
# ---------------------------------------
def run_gallery_dl(url: str, cookies: bool = False, timeout: int = 600) -> Dict[str, Any]:
    ...
    Uses archive file to avoid duplicate downloads.
    Suppress gallery-dl stdout/stderr unless an error occurs. Return result dict.
    """
    # Trim archive file before starting
    trim_archive(ARCHIVE_FILE, ARCHIVE_MAX_SIZE)
    ...
    # Build command with archive support
    cmd = [
        "gallery-dl",
        "--config", str(CONFIG_FILE),
        "--download-archive", str(ARCHIVE_FILE),
...
def main():
    ...
    # Multi-instance downloader arguments
    parser.add_argument('--multi-dl', action='store_true', help='Start multi-instance downloader interactively')
    parser.add_argument('--multi-csv', type=str, help='CSV file for multi-instance downloader')
    parser.add_argument('--multi-log', type=str, help='Log file name for multi-instance errors')
    parser.add_argument('--multi-threads', type=int, default=16, help='Number of concurrent instances (1-128)')
    ...
    # Multi-instance downloader mode
    if args.multi_dl or args.multi_csv:
        display_banner()
        ensure_directories()
        ...
        if args.multi_csv:
            # Non-interactive multi-instance mode
            csv_file = args.multi_csv
            log_file = args.multi_log or "multi_dl_errors.csv"
            threads = max(1, min(128, args.multi_threads))
            ...
            # Interactive multi-instance mode
            interactive_multi_downloader(use_cookies)
        return

    # Normal Pinterest download operation (your existing code)
    display_banner()
    ensure_directories()
    ensure_gallery_dl_config()
    if not check_installation():
    ...
    if not urls:
        ...
        print("   ptdl --multi-dl (interactive multi-instance)")
        print("   ptdl --multi-csv file.csv [--multi-log errors.csv --multi-threads 16]")
    ...
    # Print archive file and detailed report in the correct order
    print()
