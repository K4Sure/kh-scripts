#!/usr/bin/env python3
"""
Pinterest Downloader Automation Script v1.4.1
...
# Embedded gallery-dl config (the one you tested and provided)
EMBEDDED_CONFIG = {
  "extractor": {
    "pinterest": {
      "username": "",
  ...
  "output": {
    "base-directory": "~/pinterest_downloads/{user}/{board}",
...
def extract_version_from_filename(filename: str) -> tuple:
    ...
    try:
        if 'pinterest_automation_v' in filename:
            version_part = filename.split('pinterest_automation_v')[1].replace('.py', '')
...
def ensure_gallery_dl_config():
    ...
    try:
        if not CONFIG_FILE.exists():
            ...
            print(f"ℹ️  gallery-dl config already exists: {shorten_path(CONFIG_FILE)}")
        return True
    except Exception as e:
...
def download_content(url: str, username: str, use_cookies: bool = False) -> Dict[str, Any]:
    ...
    if 'pin.it' in url:
        print(f"⚠️  Warning: Shortened 'pin.it' URLs may not always resolve; prefer full pinterest.com URLs")
    ...
    # We use --config <CONFIG_FILE> (embedded) and --destination <temp_dir> for each run
    command = ["gallery-dl", "--config", str(CONFIG_FILE), "--destination", str(temp_dir), url]
    
    # Add cookies if requested and available
    if use_cookies and COOKIES_FILE.exists():
        command.insert(1, "--cookies")  # will be ["gallery-dl", "--cookies", "<path>", ...]
        command.insert(2, str(COOKIES_FILE))
