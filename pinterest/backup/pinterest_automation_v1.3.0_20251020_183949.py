#!/usr/bin/env python3
"""
Pinterest Downloader Automation Script v1.3.0
...
def list_script_versions():
    ...
    print("\n📚 Pinterest Script Versions")
    print("=" * 50)
    
    # List 1: Backed up scripts in backup directory
    print("\n💾 Backed up scripts (in backup/):")
    ...
    print("\n📂 Current scripts (in main directory):")
    ...
    if main_scripts:
        for script in sorted(main_scripts, key=lambda x: extract_version_from_filename(x.name), reverse=True):
            if script.name == current_script:
                print(f"   ✅ {script.name} (current)")
            else:
                print(f"   📄 {script.name}")
    else:
        print("   No scripts found in main directory")
...
def backup_all_scripts():
    ...
    print(f"⭐ Latest version: {latest_script.name}")
    ...
    print(f"   ⭐ Kept latest: {latest_script.name}")
