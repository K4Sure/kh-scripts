...
def backup_all_scripts():
    ...
    for script_file in script_files:
        ...
        backup_name = f"{name_without_ext}_{timestamp}.py"
        ...
        try:
            ...
            backup_failed.append(script_file.name)

    # <-- user-requested empty line after backup section
    print()

    # Clean up old versions, keeping only latest
    print(f"{BOLD}🧹 Cleaning Up Old Versions...{RESET}")
    for script_file in BASE_DIR.iterdir():
        if script_file.is_file() and script_file.name != latest_script.name and script_file.suffix == '.py':
            script_file.unlink()
    ...
    print(f"   🧹 Removed Old Versions: {len(script_files) - 1} Script(s)")
