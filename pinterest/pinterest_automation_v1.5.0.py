...
# -*- coding: utf-8 -*-
"""
Pinterest Downloader Automation Script v1.5.0
Gallery-dl backend, embedded config, full backup manager, ANSI colors and emojis preserved.
Multi-instance downloader with hardcoded CSV, auto thread selection, and automatic title extraction.
Place this file at:
 /data/data/com.termux/files/home/kh-scripts/pinterest/pinterest_automation_v1.5.0.py
"""
...
# -------------------------
class ThreadProgressManager:
    def __init__(self):
        self.indicators = {}
        self.lock = threading.Lock()
    
    def create_indicator(self, thread_id, total_files, folder_name):
        with self.lock:
            self.indicators[thread_id] = {
                'indicator': ProgressIndicator(total_files),
                'folder_name': folder_name,
                'line_position': len(self.indicators)
            }
    
    def update(self, thread_id, count=1):
        with self.lock:
            if thread_id in self.indicators:
                self.indicators[thread_id]['indicator'].update(count)
    
    def finish(self, thread_id):
        with self.lock:
            if thread_id in self.indicators:
                self.indicators[thread_id]['indicator'].finish()
                del self.indicators[thread_id]

# Create global progress manager
progress_manager = ThreadProgressManager()
...
# -------------------------
def extract_title_from_pinterest_url(url: str) -> str:
    """
    Extract a meaningful title from Pinterest URL.
    Returns formatted title like 'Board Name by Username' or 'Username Profile'
    """
    try:
        ...
        if not parts:
            return "Unknown Pinterest Content"
        
        # Handle different Pinterest URL patterns
        if len(parts) >= 2:
            # Pattern: /username/boardname/
            username = parts[0]
            board_name = parts[1]
            
            # Decode URL-encoded characters and replace hyphens with spaces
            board_name = urllib.parse.unquote(board_name).replace('-', ' ').replace('_', ' ')
            username = urllib.parse.unquote(username).replace('-', ' ').replace('_', ' ')
            
            # Capitalize words for better readability
            board_name = ' '.join(word.capitalize() for word in board_name.split())
            username = ' '.join(word.capitalize() for word in username.split())
            
            return f"{board_name} by {username}"
        ...
        print(f"{RED}❌ Error extracting title from URL: {e}{RESET}")
...
def validate_and_extract_csv_titles(csv_file: Path) -> List[Dict[str, str]]:
    """
    Read CSV and ensure every row has a title.
    If title is missing, extract from URL automatically.
    """
    processed_rows = []
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            if 'url' not in fieldnames:
                print(f"{RED}❌ CSV must contain 'url' column{RESET}")
                return []
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                if not url:
                    print(f"{RED}❌ Row {row_num}: Missing URL{RESET}")
                    continue
                
                # Extract or use existing title
                if has_title_column and row.get('title', '').strip():
                    title = row['title'].strip()
                else:
                    title = extract_title_from_pinterest_url(url)
                    print(f"{NEON_YELLOW}📝 Auto-generated title for row {row_num}: {title}{RESET}")
...
class ProgressIndicator:
    ...
    def _display(self):
        ...
        # Use file_lock instead of print_lock for progress bars to allow concurrent updates
        with file_lock:
...
def organize_downloaded_files(download_dir: Path, username: str, thread_id=None):
    ...
    if thread_id is None:
        ...
        # Multi-thread mode - minimal output
        with print_lock:
            print(f'{BOLD}[Thread]{RESET} {NEON_CYAN}Organizing:{RESET} {username}')
    ...
    # Initialize progress indicator based on mode
    if thread_id is None:
        progress = ProgressIndicator(total_files)
    else:
        progress_manager.create_indicator(thread_id, total_files, username)
            ...
            if file_path.is_file():
                ...
                try:
                    ...
                    if thread_id is None:
                        ...
                        progress_manager.update(thread_id, 1)
                except Exception:
                    if thread_id is None:
                        progress.update(1)
                    else:
    ...
    if thread_id is None:
        ...
        progress_manager.finish(thread_id)
...
def check_cookies_available() -> bool:
    """Check if cookies file exists and is valid Netscape format"""
    if COOKIES_FILE.exists():
        try:
            # For Netscape format, check if file has content and basic structure
            with open(COOKIES_FILE, 'r', encoding='utf-8') as f:
                content = f.read().strip()
            
            # Basic validation for Netscape cookie format
            if content and any(line.startswith(('.pinterest.com', 'www.pinterest.com')) for line in content.split('\n') if line.strip() and not line.startswith('#')):
                return True
            else:
                return False
        except Exception:
...
def run_multi_command(folder_name: str, image_url: str, log_file: str, line_number: int, total_lines: int, use_cookies: bool = False):
    ...
    with print_lock:
        print(f'{BOLD}[{line_number}/{total_lines}]{RESET} {NEON_CYAN}Processing:{RESET} {NEON_PINK}{folder_name}{RESET}')
    ...
    if result == 0:
        ...
        organized_files = organize_downloaded_files(temp_dir, sanitized_folder_name, thread_id)
...
def run_multi_instance_downloader(use_cookies: bool = False):
    ...
    print()
    print("=" * 60)
    print(f"{BOLD}🚀 Multi-Instance Downloader v1.5.0{RESET}")
    print("=" * 60)
    print()
    ...
    if not MULTI_DL_CSV_FILE.exists():
        ...
        print(f"{NEON_YELLOW}💡 Please Create the CSV File with 'url' column (title column is optional){RESET}")
        return
    ...
    # Process CSV with automatic title extraction
    print(f"{NEON_YELLOW}📊 Processing CSV and extracting titles from URLs...{RESET}")
    processed_rows = validate_and_extract_csv_titles(MULTI_DL_CSV_FILE)
    
    if not processed_rows:
        print(f"{RED}❌ No valid rows found in CSV file{RESET}")
        return

    total_lines = len(processed_rows)
    ...
    print()
    print(f"{NEON_GREEN}✅ Auto-title extraction completed!{RESET}")
    print()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        futures = []
        for line_number, row in enumerate(processed_rows, start=1):
...
def main():
    ...
    if not urls:
        ...
        print("   ptdl --multi-dl (Hardcoded CSV + Auto Threads + Auto Title Extraction)")
